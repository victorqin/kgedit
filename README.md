...
npm run e2e         # 端到端，Chrome + Firefox + Safari
```
...
- **E2E 并发限制为 2，并保留一次重试**。三个浏览器引擎各自跑力导向布局，单机 CPU 争用会让图渲染的等待偶发超时；这类用例单独跑都稳定通过。真实缺陷在重试后依然会失败。
...
判定逻辑收敛在 `graph/graphOptions.ts` 的 `isMiddleButtonDrag`，`drag-canvas` 与 `drag-element` 共用。要注意 G6 合成的 `dragstart` 事件上 `button` 恒为 `-1`，能区分按键的只有 `buttons` 位掩码（1=左 / 2=右 / 4=中）；上面那个补发的 dragstart 发生时 `buttons` 为 `0`，因此一并被挡掉。
...
**双击同时承担两件事**：重新聚焦与选中。两个请求互不依赖，并行发出；双击前置的那次单击由 250ms 去重窗口挡掉，所以关系列表只会被请求一次。
